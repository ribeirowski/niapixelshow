...
class CartController {
    async get(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const cartDoc = await firestoreDBTest.collection('carts').doc(userId).get();
            if (!cartDoc.exists) {
    ...
    async add_item(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!cartDoc.exists) {
                ...
                await firestoreDBTest.collection('carts').doc(userId).set(cartData);
            ...
            await firestoreDBTest.collection('carts').doc(userId).update({ items: updatedItems });

            const cartPrice = updatedItems.reduce((acc: number, item: any) => acc + item.price*item.quantity, 0);
            await firestoreDBTest.collection('carts').doc(userId).update({ price: cartPrice });
    ...
    async delete(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            await firestoreDBTest.collection('carts').doc(userId).delete();
