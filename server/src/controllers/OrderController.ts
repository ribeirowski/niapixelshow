...
class OrderController {
  ...
  async getStats(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const allOrders = await firestoreDB
        .collection("orders")
        .where("status", "==", "Pago")
        .get();
      ...
      const orders = ordersRaw
        .map((element) => {
          ...
          if (
            (compare[0] === year && compare[1] === month) ||
            year === "0000"
          ) {
            return orderVar;
          }
  ...
  async filterAll(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (email === "") {
        if (func === "Acima de") {
          var allFiltOrders = await firestoreDB
            .collection("orders")
            .where(atribute, ">", filt)
            .get();
        ...
        allOrdersSnapshot = await firestoreDB
          .collection("orders")
          .where("email", "==", email)
          .get();
        ...
        if (func === "Acima de") {
          filteredOrders = orders.filter(
            (order) => order[atribute as keyof Pedido] > filt
          );
        } else if (func === "Abaixo de") {
          filteredOrders = orders.filter(
            (order) => order[atribute as keyof Pedido] < filt
