...
class PromotionController {
    ...
    async create(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const promotionQuery = await firestoreDB.collection('promotions').where('name', '==', promotionData.name).get();
            ...
            // id promoção
            const promotionDoc = await firestoreDB.collection('promotions').where('name', '==', promotionData.name).get();
            const id = promotionDoc.docs[0].id;

            // Atualiza o produto com a referência da promoção
            await firestoreDB.collection('products').doc(promotionData.product_id).update({ promotionId: id });
            ...
            await firestoreDB.collection('products').doc(promotionData.product_id).update({ price: newPrice });
            
        
            res.status(201).json({ message: 'Promotion created successfully', id: id });
            return next();
        } catch (error) {
            return next(error);
        }
    ...
    async delete(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            // Atualiza o produto com a referência da promoção
            await firestoreDB.collection('products').doc(productId).update({ promotionId: null });
