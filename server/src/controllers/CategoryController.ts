  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!name) {
        return res
          .status(400)
          .json({ message: "O nome da categoria deve ser preenchidos!" });
      }
      const categoryData = Category.parse(req.body);

      const sameCategoryName = await firestoreDB
      ...
      const categoryRecord = await firestoreDB
        .collection("categories")
        .add(categoryData);
    ...
    try {
      ...
      res
        ...
        .json({
          ...
          product: { ...categoryDoc.data(), ...categoryData },
        });
      return next();
    } catch (error) {
      return next(error);
