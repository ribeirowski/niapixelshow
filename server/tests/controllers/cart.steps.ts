...
defineFeature(feature, (test) => {
  ...
  test("Adicionar um produto ao carrinho vazio", ({
    given,
    when,
    then,
    and,
    ...
    when(
      /^o produto com item_id "(.*)", nome "(.*)", descrição "(.*)", preço "(.*)", status "(.*)", categoria "(.*)", quantidade "(.*)" e tamanho "(.*)" é adicionado ao carrinho pelo user de id "(.*)"$/,
      async (arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8) => {
        const loginRespose = await request.post("/auth/login").send({
          email: "nrc2@cin.ufpe.br",
          password: "nia12345",
        });
        token = loginRespose.headers["set-cookie"];
        response = await request.delete(`/cart/5`).set("Cookie", token);
        const orderData = {
          item_id: arg0,
          name: arg1,
          description: arg2,
    ...
    then(
      /^o banco de dados cria um novo carrinho com user_id "(.*)" e o produto de item_id "(.*)"$/,
      async (arg0, arg1) => {
        response = await request.get(`/cart/${arg0}`).set("Cookie", token);
        expect(response.body.items[0].item_id).toBe(arg1);
      }
    );
    and(
      'o preço total do carrinho de user_id "5" é atualizado para o valor do produto de item_id "1"',
      async () => {
        response = await request.get(`/cart/5`).set("Cookie", token);
        const price_item =
          response.body.items[0].price * response.body.items[0].quantity;
  ...
  test("Adicionar um produto ao carrinho com produtos", ({
    given,
    when,
    then,
    and,
    ...
    given(
      'o banco de dados possui um carrinho com user_id "6" e o produto de item_id "1" com nome "Camisa Cin 50 anos", descrição "Esta é uma camisa que representa os 50 anos do Centro de informática", preço "50", status "true", categoria "Camisas", quantidade "4" e tamanho "P"',
      async () => {
        response = await request.delete(`/cart/6`).set("Cookie", token);
        const orderData = {
          item_id: "1",
          name: "Camisa Cin 50 anos",
          description:
            "Esta é uma camisa que representa os 50 anos do Centro de informática",
          price: 50,
          status: true,
          category: {
            name: "Camisas",
    ...
    when(
      /^o produto com item_id "(.*)", nome "(.*)", descrição "(.*)", preço "(.*)", status "(.*)", categoria "(.*)", quantidade "(.*)" e tamanho "(.*)" é adicionado ao carrinho pelo usuário de user_id "(.*)"$/,
      async (arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8) => {
        const orderData = {
          item_id: arg0,
          name: arg1,
    ...
    then(
      /^o banco de dados adiciona o produto de item_id "(.*)" ao carrinho de user_id "(.*)"$/,
      async (arg0, arg1) => {
        response = await request.get(`/cart/${arg1}`).set("Cookie", token);
        expect(response.body.items[1].item_id).toBe(arg0);
      }
    );
    and(
      'o preço total do carrinho de user_id "6" é atualizado para a soma dos valores dos produtos',
      async () => {
        response = await request.get(`/cart/6`).set("Cookie", token);
        const price_item1 =
          response.body.items[0].price * response.body.items[0].quantity;
        const price_item2 =
          response.body.items[1].price * response.body.items[1].quantity;
  ...
  test("Remover um produto do carrinho", ({ given, when, then, and }) => {
    given(
      'o banco de dados possui um carrinho com user_id "8" e o produto de item_id "1" com nome "Camisa Cin 50 anos", descrição "Esta é uma camisa que representa os 50 anos do Centro de informática", preço "50", status "true", categoria "Camisas", quantidade "4" e tamanho "P"',
      async () => {
        response = await request.delete(`/cart/8`).set("Cookie", token);
        const orderData = {
          item_id: "1",
          name: "Camisa Cin 50 anos",
          description:
    ...
    when(
      /^o produto de item_id "(.*)" é removido do carrinho$/,
      async (arg0) => {
        response = await request.get(`/cart/8`).set("Cookie", token);
        response = await request.delete(`/cart/8/${arg0}`).set("Cookie", token);
      }
    );
    then(
      /^o banco de dados remove o produto de item_id "(.*)" do banco de dados$/,
      async (arg0) => {
        response = await request.get(`/cart/8`).set("Cookie", token);
    ...
    and(
      'o preço do produto de item_id "1" é subtraido do preço total do carrinho de user_id "8"',
      async () => {
        response = await request.get(`/cart/8`).set("Cookie", token);
  ...
  test("Editar um produto do carrinho", ({ given, when, then, and }) => {
    given(
      'o banco de dados possui um carrinho com user_id "10" e o produto de item_id "1" com nome "Camisa Cin 50 anos", descrição "Esta é uma camisa que representa os 50 anos do Centro de informática", preço "50", status "true", categoria "Camisas", quantidade "4" e tamanho "P"',
      async () => {
        response = await request.delete(`/cart/10`).set("Cookie", token);
        const orderData = {
          item_id: "1",
          name: "Camisa Cin 50 anos",
          description:
    ...
    when(
      /^o tamanho do produto de item_id "(.*)" é editado para "(.*)"$/,
      async (arg0, arg1) => {
        const orderData = {
          size: arg1,
        };
    ...
    and(
      /^a quantidade do produto de item_id "(.*)" é editado para "(.*)"$/,
      async (arg0, arg1) => {
        const orderData = {
    ...
    then(
      /^o banco de dados edita o valor do campo tamanho do produto de item_id "(.*)" para "(.*)"$/,
      async (arg0, arg1) => {
        response = await request.get(`/cart/10`).set("Cookie", token);
        expect(response.body.items[0].size).toBe("M");
      }
    );
    and(
      /^o banco de dados edita o valor do campo de quantidade do produto de item_id para "(.*)"$/,
      async (arg0) => {
        response = await request.get(`/cart/10`).set("Cookie", token);
  ...
  test("Finalizar o pedido no carrinho de compras", ({ given, when, then }) => {
    given(
      'o banco de dados possui um carrinho com user_id "12" e o produto de item_id "1" com nome "Camisa Cin 50 anos", descrição "Esta é uma camisa que representa os 50 anos do Centro de informática", preço "50", status "true", categoria "Camisas", quantidade "4" e tamanho "P"',
      async () => {
        const orderData = {
          item_id: "1",
          name: "Camisa Cin 50 anos",
    ...
    when("o pedido é finalizado no carrinho", async () => {
      response = await request.delete(`/cart/12`).set("Cookie", token);
    });
    then(
      /^o banco de dados deleta o carrinho de user_id "(.*)"$/,
      async (arg0) => {
        response = await request.get(`/cart/${arg0}`).set("Cookie", token);
