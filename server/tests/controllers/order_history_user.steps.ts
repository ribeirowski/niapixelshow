...
defineFeature(feature, (test) => {
  ...
  test("Retornar pedidos no histórico de pedidos com pedidos cadastrados", ({
    given,
    and,
    when,
    then,
    ...
    when("acessar a página de Histórico de Pedidos", async () => {
      ...
      response = await request.get(
        `/order/filter/email?func=${func}&filter=${filter}&email=${usedEmail}`
      );
      //console.log(response.body);
    });
    then(
      /^é retornado o pedido com email "(.*)", item "(.*)" com descrição "(.*)", quantidade "(.*)", preço "(.*)" reais, status "(.*)", criado em "(.*)", para o endereço "(.*)"$/,
      async (arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7) => {
  ...
  test("Retornar mensagem no histórico de pedidos sem pedidos cadastrados", ({
    given,
    and,
    when,
    then,
    ...
    then(
      "é retornada uma mensagem informando que não há pedidos cadastrados",
      async () => {
  ...
  test("Retornar pedidos filtrados", ({ given, and, when, then }) => {
    ...
    when(/^filtrar por "(.*)" "(.*)"$/, async (arg0, arg1) => {
      ...
      response = await request.get(
        `/order/filter/${arg0}?func=${func}&filter=${filter}&email=${usedEmail}`
      );
    });
    then(
      /^é retornado o pedido com email "(.*)", item "(.*)" com descrição "(.*)", quantidade "(.*)", preço "(.*)" reais, status "(.*)", criado em "(.*)", para o endereço "(.*)"$/,
