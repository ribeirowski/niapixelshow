...
defineFeature(feature, (test) => {
  ...
  test("Administrador Adiciona uma nova Categoria de Produto", ({
    given,
    when,
    then,
    and,
    ...
    given(
      /^o administrador de nome ligia, e mail ligiaferropadilha@gmail.com e senha “liginha(\d+)” está logado no painel de administração do e-commercen na página "(.*)"$/,
      async (arg0, arg1) => {
    ...
    and(
      /^o sistema valida que os campos "(.*)", "(.*)" estão válidos$/,
      async (arg0, arg1) => {
        expect(response.body.category.name).toBe("Camisas");
        expect(response.body.category.description).toBe(
          "Camisas temáticas do centro de informática"
        );
      }
    );

    then(
      "o sistema salva a categoria e o administrador recebe uma confirmação de sucess",
      () => {
  ...
  test("Erro ao Adicionar uma nova Categoria de Produto", ({
    given,
    when,
    then,
    and,
    ...
    and("uma mensagem de erro é retornada", () => {
      expect(response.body.message).toBe(
        "O nome da categoria deve ser preenchidos!"
      );
    });
  });

  test("Administrador Edita uma Categoria Existente", ({
    given,
    when,
    then,
    and,
    ...
    when(
      'ele seleciona a opção “editar” para a categoria “Camisetas Personalizadas” e altera seu nome para "Camisetas Temáticas"',
      async () => {
        const updateData = {
          name: "Camisetas Temáticas",
        };
        response = await request
          .put("/category/Camisetas Personalizadas")
          .send(updateData);
      }
    );
    ...
    and(
      "o sistema salva a categoria atualizada e retorna uma mensagem de confirmação",
      () => {
  ...
  test("Administrador Remove uma Categoria Existente", ({
    given,
    when,
    then,
  }) => {
    ...
    when(
      "ele seleciona a opção “remover” para a categoria “Promoções Temporárias”",
      async () => {
    ...
    then(
      "o sistema salva a remoção da categoria e retorna uma mensagem de confirmação",
      () => {
        expect(response.status).toBe(200);
